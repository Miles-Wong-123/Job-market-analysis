...
from job_market.adapters.xiaomi import XiaomiAdapter

FIXTURE = Path(__file__).parent.parent / "fixtures" / "xiaomi_jobs_p1.json"
...
def test_lists_with_recruit_type_1(page1: dict[str, Any]) -> None:
    page1["data"]["total"] = 5
    adapter = XiaomiAdapter()
    fetcher = FakeFetcher(page1)
    raws = list(adapter.list_jobs(fetcher))  # type: ignore[arg-type]
    assert len(raws) == 5
    assert all(p["recruitType"] == 1 for p in fetcher.posts)


def test_normalize_two_cities(page1: dict[str, Any]) -> None:
    adapter = XiaomiAdapter()
    n = adapter.normalize(RawJob(source_url="x", payload=page1["data"]["list"][2]))
    assert n.job_id == "xiaomi:XM-2003"
    assert n.location == ["北京", "武汉"]
    assert n.posted_at == "2026-06-06"
